	...
	<% if(paging != null){ %>
		<div class="center">
			<div class="pagination">
				<% if(paging.isPrev()){ %>
					<a href="/sale_share/searchList?search=<%=request.getParameter("search") %>&search_text=<%=request.getParameter("search_text")%>&nowPage=<%=(paging.getPageBarStart()-1)%>">
						&laquo;
					</a>
				<%}%>
				<% for(int i = paging.getPageBarStart() ; i <= paging.getPageBarEnd() ; i++) {%>
					<a href="/sale_share/searchList?search=<%= request.getParameter("search") %>&search_text=<%= request.getParameter("search_text") %>&nowPage=<%=i%>" 
					<%=paging.getNowPage() == i ? "class='active'" : ""%>>
						<%=i%>
					</a>
				<%}%>
				<% if(paging.isNext()){%>
					<a href="/sale_share/searchList?search=<%=request.getParameter("search")%>&search_text=<%=request.getParameter("search_text") %>&nowPage=<%=(paging.getPageBarEnd()+1)%>">&raquo;</a>
				<%}%>
			</div>
		</div>
	<% } %>
